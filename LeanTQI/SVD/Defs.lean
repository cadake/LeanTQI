...
--     (isHermitian_transpose_mul_self A).eigenvectorUnitary).toColumns₁
...
--   (reindex
--     (finRankEquivDecEigsConjTransposeMulSelf A).symm
--     (finRankEquivDecEigsConjTransposeMulSelf A).symm)
--   (diagonal (fun (i : {a // (decEigenvalues (isHermitian_transpose_mul_self A)) a ≠ 0}) =>
...
-- noncomputable def svdμ' (A : Matrix (Fin M) (Fin N) 𝕂) : Matrix (Fin A.rank) (Fin A.rank) ℝ :=
--   (reindex
--     (finRankEquivDecEigsMulConjTranspose A).symm
--     (finRankEquivDecEigsMulConjTranspose A).symm)
--   (diagonal (fun (i : {a // (decEigenvalues (isHermitian_mul_conjTranspose_self A)) a ≠ 0}) =>
...
--     (isHermitian_mul_conjTranspose_self A).eigenvectorUnitary).toColumns₁
...
-- lemma reindex_eigenRowEquiv_eigenvectorUnitary (A : Matrix (Fin M) (Fin N) 𝕂) :
--   ((reindex (Equiv.refl (Fin M)) (eigenRowEquiv A))
--     (isHermitian_mul_conjTranspose_self A).eigenvectorUnitary) = fromColumns A.svdU₁' A.svdU₂ := by
--   rw [svdU₂, svdU₁']
...
-- lemma reindex_eigenRowEquiv_eigenvectorUnitary' (A : Matrix (Fin M) (Fin N) 𝕂) :
--   ((reindex (Equiv.refl (Fin N)) (eigenColumnEquiv A))
--     (isHermitian_transpose_mul_self A).eigenvectorUnitary) = fromColumns A.svdV₁ A.svdV₂ := by
--   rw [svdV₁, svdV₂]
--   simp only [reindex_apply, Equiv.refl_symm, Equiv.coe_refl, fromColumns_toColumns]

-- lemma reindex_eigenRowEquiv_eigenvectorUnitary'' (A : Matrix (Fin M) (Fin N) 𝕂) :
--   (reindex A.eigenColumnEquiv (Equiv.refl (Fin N)))
--     ((isHermitian_transpose_mul_self A).eigenvectorUnitary)ᴴ = (fromColumns A.svdV₁ A.svdV₂)ᴴ := by
--   rw [svdV₁, svdV₂]
--   simp only [reindex_apply, Equiv.refl_symm, Equiv.coe_refl, fromColumns_toColumns, conjTranspose_submatrix]
...
-- noncomputable def eigenvectorUnitaryInv : Matrix n n 𝕜 :=
--   (eigenvectorBasis hA).toBasis.toMatrix (PiLp.basisFun _ 𝕜 n)
-- -- #align matrix.is_hermitian.eigenvector_matrix_inv Matrix.IsHermitian.eigenvectorUnitaryInv

-- theorem eigenvectorUnitary_mul_inv : hA.eigenvectorUnitary * hA.eigenvectorUnitaryInv = 1 := by
--   apply Basis.toMatrix_mul_toMatrix_flip
-- -- #align matrix.is_hermitian.eigenvector_matrix_mul_inv Matrix.IsHermitian.eigenvectorUnitary_mul_inv

-- noncomputable instance : Invertible hA.eigenvectorUnitaryInv :=
--   invertibleOfLeftInverse _ _ hA.eigenvectorUnitary_mul_inv

-- noncomputable instance : Invertible (hA.eigenvectorUnitary.val) :=
--   invertibleOfRightInverse _ _ hA.eigenvectorUnitary_mul_inv

-- -- theorem eigenvectorUnitary_apply (i j : n) : hA.eigenvectorUnitary i j = hA.eigenvectorBasis j i := by
-- --   simp_rw [eigenvectorUnitary, Basis.toMatrix_apply, OrthonormalBasis.coe_toBasis,
-- --     PiLp.basisFun_repr]
...
-- theorem eigenvectorUnitaryInv_apply (i j : n) :
--     hA.eigenvectorUnitaryInv i j = star (hA.eigenvectorBasis i j) := by
--   rw [eigenvectorUnitaryInv, Basis.toMatrix_apply, OrthonormalBasis.coe_toBasis_repr_apply,
...
-- -- #align matrix.is_hermitian.eigenvector_matrix_inv_apply Matrix.IsHermitian.eigenvectorUnitaryInv_apply

-- theorem conjTranspose_eigenvectorUnitaryInv : hA.eigenvectorUnitaryInvᴴ = hA.eigenvectorUnitary := by
--   ext i j
--   rw [conjTranspose_apply, eigenvectorUnitaryInv_apply, eigenvectorUnitary_apply, star_star]
--   rfl
-- -- #align matrix.is_hermitian.conj_transpose_eigenvector_matrix_inv Matrix.IsHermitian.conjTranspose_eigenvectorUnitaryInv

-- theorem conjTranspose_eigenvectorUnitary : hA.eigenvectorUnitaryᴴ = hA.eigenvectorUnitaryInv := by
--   rw [← conjTranspose_eigenvectorUnitaryInv, conjTranspose_conjTranspose]
...
--     IsHermitian.conjTranspose_eigenvectorUnitary, ← mul_apply,
--     Matrix.mul_eq_one_comm.1 (IsHermitian.eigenvectorUnitary_mul_inv _), one_apply,
...
--     ← conjTranspose_apply, IsHermitian.conjTranspose_eigenvectorUnitary, ← mul_apply,
--     Matrix.mul_eq_one_comm.1 (IsHermitian.eigenvectorUnitary_mul_inv _), one_apply,
...
--   simp only [ne_eq, reindex_apply, Equiv.symm_symm, submatrix_diagonal_equiv, diagonal_transpose]
...
--   simp only [diagonal_apply, Sum.inl.injEq, Function.comp_apply, ne_eq, reindex_apply,
--     Equiv.symm_symm, submatrix_diagonal_equiv, fromBlocks_apply₁₁]
--   -- sorry
--   -- sorry
--   -- sorry
--   -- sorry
--   rfl
--   simp only [↓reduceIte, fromBlocks_apply₂₁, zero_apply]

--   simp only [reduceCtorEq, ↓reduceIte, fromBlocks_apply₁₂, zero_apply]
--   rfl
--   simp only [Sum.inr.injEq, eigen_eigenColumnEquiv_inr, ite_self, fromBlocks_apply₂₂, zero_apply]
...
--   simp only [↓reduceIte, fromBlocks_apply₂₁, zero_apply]
--   rfl
--   -- simp only [Sum.inr.injEq, eigen_eigenRowEquiv_inr, ite_self, fromBlocks_apply₂₂, zero_apply]
--   simp only [reduceCtorEq, ↓reduceIte, fromBlocks_apply₁₂, zero_apply]
--   simp only [Sum.inr.injEq, eigen_eigenRowEquiv_inr, ite_self, fromBlocks_apply₂₂, zero_apply]
--   sorry


-- lemma reindex_eigenColumnEquiv_eigenvectorUnitary (A : Matrix (Fin M) (Fin N) 𝕂) :
--     (reindex (Equiv.refl (Fin N)) (eigenColumnEquiv A))
--       (isHermitian_transpose_mul_self A).eigenvectorUnitary =
...
-- #check star_eq_conjTranspose
-- #check fromColumns_mul_fromBlocks
...
--     ← IsHermitian.conjTranspose_eigenvectorUnitaryInv, Matrix.mul_assoc,
--     ← submatrix_mul_equiv
--       hAHA.eigenvectorUnitaryInvᴴ (diagonal (RCLike.ofReal ∘ hAHA.eigenvalues)  *
--       (star hAHA.eigenvectorUnitaryInvᴴ)) _ (eigenColumnEquiv A).symm _,
--     ← submatrix_mul_equiv (diagonal (RCLike.ofReal ∘ hAHA.eigenvalues)) (star hAHA.eigenvectorUnitaryInvᴴ)
--     _ (eigenColumnEquiv A).symm _,
--     ← @RCLike.algebraMap_eq_ofReal 𝕂]
--   sorry
--   -- simp_rw [Function.comp]
--   -- rw [← diagonal_map, submatrix_map, ← reindex_apply, ← Equiv.coe_refl, ← Equiv.refl_symm,
--   --   ← reindex_apply, star_eq_conjTranspose, conjTranspose_conjTranspose, ← reindex_apply, μ_block,
--   --   IsHermitian.conjTranspose_eigenvectorUnitaryInv, reindex_eigenColumnEquiv_eigenvectorUnitary,
--   --   ← Matrix.mul_assoc, fromBlocks_map, fromColumns_mul_fromBlocks,
--   --   ← IsHermitian.conjTranspose_eigenvectorUnitary, reindex_eigenRowEquiv_eigenvectorUnitary'', conjTranspose_fromColumns_eq_fromRows_conjTranspose, fromColumns_mul_fromRows]
--   -- simp only [map_zero, Matrix.map_zero, Matrix.zero_mul, add_zero, Matrix.mul_zero]
--   -- -- rw [Matrix.mul_assoc]
--   -- apply map_zero

-- -- todo : lean 定义了 star M 是 conjTranspose
...
--     ← IsHermitian.conjTranspose_eigenvectorUnitaryInv, Matrix.mul_assoc,
--     ← submatrix_mul_equiv
--       hAAH.eigenvectorUnitaryInvᴴ (diagonal (RCLike.ofReal ∘ hAAH.eigenvalues) *
--       (star hAAH.eigenvectorUnitaryInvᴴ)) id (eigenRowEquiv A).symm id,
--     ← submatrix_mul_equiv (diagonal (RCLike.ofReal ∘ hAAH.eigenvalues)) (star hAAH.eigenvectorUnitaryInvᴴ)
--     _ (eigenRowEquiv A).symm _,
--       ← @RCLike.algebraMap_eq_ofReal 𝕂]
--   sorry
--   -- simp_rw [Function.comp]
--   -- rw [← diagonal_map, submatrix_map,
--   --   ← reindex_apply, ← Equiv.coe_refl, ← Equiv.refl_symm, ← reindex_apply,
--   --   star_eq_conjTranspose, ← conjTranspose_submatrix, ← reindex_apply, μ'_block,
--   --   IsHermitian.conjTranspose_eigenvectorUnitaryInv, reindex_eigenRowEquiv_eigenvectorUnitary,
...
--   have := (Real.sqrt_ne_zero (eigenvalues_conjTranspose_mul_self_nonneg _ _)).2
--     ((finRankEquivEigsConjTransposeMulSelf A).1 i).prop
--   apply (Real.sqrt_ne_zero (eigenvalues_conjTranspose_mul_self_nonneg _ _)).2
--   have := ((finRankEquivDecEigsConjTransposeMulSelf A).1 i).prop
--   simp only [decEigenvalues, eq_mpr_eq_cast, List.get_eq_getElem, ne_eq]
--   have : ¬(isHermitian_transpose_mul_self A).eigenvalues
--     (A.finRankEquivEigsConjTransposeMulSelf.toFun i) = 0 := by
--     apply ((finRankEquivEigsConjTransposeMulSelf A).1 i).prop
--   sorry
--   -- have :=
--   -- rw [decEigsConjMulEquivEigsConjMul]


--   -- exact (Real.sqrt_ne_zero (eigenvalues_conjTranspose_mul_self_nonneg _ _)).2
--   --   ((finRankEquivDecEigsConjTransposeMulSelf A).1 i).prop
...
--   intro i
--   simp only [Finset.mem_univ, ne_eq, map_eq_zero, forall_true_left, Equiv.symm_symm]
--   sorry
--   -- apply (Real.sqrt_ne_zero (eigenvalues_conjTranspose_mul_self_nonneg _ _)).2
--   --     ((finRankEquivEigsConjTransposeMulSelf A).1 i).prop
...
--     A * A.svdV₂ = 0 := by
--   suffices h : Aᴴ * A * A.svdV₂ = 0 by exact (conjTranspose_mul_self_mul_eq_zero _ _).1 h
...
--   intros x
--   rw [RCLike.star_def, RCLike.algebraMap_eq_ofReal, starRingEnd_apply, star_trivial,
...
--     ← conjTranspose_apply, IsHermitian.conjTranspose_eigenvectorUnitary,
--     ← Matrix.mul_apply, Matrix.mul_eq_one_comm.1 (IsHermitian.eigenvectorUnitary_mul_inv _)]
--   by_cases hij: i = j
--   · rw [hij]
...
--     submatrix_apply, id_eq, HMul.hMul, dotProduct, conjTranspose_apply, of_apply,
--     ← conjTranspose_apply, IsHermitian.conjTranspose_eigenvectorUnitary, ← mul_apply,
--     Matrix.mul_eq_one_comm.1 (IsHermitian.eigenvectorUnitary_mul_inv _)]
--   funext i j
--   simp only [ne_eq, EmbeddingLike.apply_eq_iff_eq, reduceCtorEq, not_false_eq_true, one_apply_ne,
--     zero_apply]

-- lemma conjTranspose_mul_U₂_eq_zero (A : Matrix (Fin M) (Fin N) 𝕂) : Aᴴ * A.svdU₂ = 0 := by
--   suffices h : A * Aᴴ * A.svdU₂ = 0 by exact (self_mul_conjTranspose_mul_eq_zero _ _).1 h
